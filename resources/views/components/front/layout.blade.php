...
<script>
        let eduction_counter = 1; // Initial counter for unique field names

        document.getElementById('addEducationBtn').addEventListener('click', function(event) {
            event.preventDefault(); // Prevent default behavior

            let eduction_container = document.getElementById('educationContainer');

            if (eduction_counter < 5) { // Limit to 10 sets
                eduction_counter++; // Increment counter

                // Create a new wrapper div
                let eduction_docWrapper = document.createElement('div');
                eduction_docWrapper.className = "p-4 border border-gray-300 rounded-lg flex flex-col gap-2";

                // Document Name Field
                let eduction_documentField = `
                    <div class="w-full flex flex-col gap-1">
                        <label for="eductionname${eduction_counter}" class="font-semibold text-gray-700 text-sm">Education Name</label>
                        <div class="w-full relative">
                            <input type="text" name="eductionname${eduction_counter}" id="eductionname${eduction_counter}" placeholder="Education name..."
                                class="w-full pl-2 pr-8 py-1 rounded-[3px] rounded-tr-[8px] border-[1px] border-b-[2px] border-r-[2px] border-secondary/40 focus:outline-none focus:ring-0 focus:border-secondary/70 placeholder-ternary/70 transition ease-in duration-2000">
                            <i class="fa fa-link absolute right-3 top-[50%] translate-y-[-50%] text-sm text-gray-500"></i>
                        </div>
                    </div>`;
                ...
                // File Upload Field
                let eduction_fileField = `
                    <div class="w-full flex flex-col gap-1">
                        <label for="eduction${eduction_counter}" class="font-semibold text-gray-700 text-sm">Marksheet</label>
                        <div class="w-full relative">
                            <input type="file" name="eductionfile${eduction_counter}" id="eductionfile${eduction_counter}"
                                class="w-full pl-2 pr-8 py-1 rounded-[3px] rounded-tr-[8px] border-[1px] border-b-[2px] border-r-[2px] border-secondary/40 focus:outline-none focus:ring-0 focus:border-secondary/70 placeholder-ternary/70 transition ease-in duration-2000">
                            <i class="fa fa-database absolute right-3 top-[50%] translate-y-[-50%] text-sm text-gray-500"></i>
                        </div>
                    </div>`;
                ...
                // Remove Button
                let eduction_removeBtn = `
                    <button type="button" class="mt-2 px-4 py-1 bg-red-500 text-black rounded remove-btn">Remove</button>
                `;

                // Append fields and button to the wrapper
                eduction_docWrapper.innerHTML = eduction_documentField + eduction_fileField + eduction_removeBtn;

                // Append the wrapper to the container
                eduction_container.appendChild(eduction_docWrapper);

                // Add event listener to remove the section
                eduction_docWrapper.querySelector(".remove-btn").addEventListener('click', function() {
                    eduction_docWrapper.remove();
                });
