...
@php
    // Detect active states for parent modules
    $currentRoute = Route::currentRouteName();
    
    // Services should only be active for actual service routes, not booking routes
    $isServicesActive = in_array($currentRoute, array_values($services->toArray() ?? [])) && 
                       !in_array($currentRoute, ['flight.booking', 'superadminhotel.booking', 'agencyvisa.booking']);
    
    $isVisaActive = in_array($currentRoute, ['agency.application']) || 
                   str_contains($currentRoute, 'agency.application');
    
    $isInvoiceActive = in_array($currentRoute, ['invoice.all']) || 
                      str_contains($currentRoute, 'invoice');
    
    $isBookingActive = in_array($currentRoute, ['flight.booking', 'superadminhotel.booking', 'agencyvisa.booking']);
    
    $isClientActive = in_array($currentRoute, ['client.index', 'client.account', 'client.pending', 'client.downloadcenter']) || 
                     str_contains($currentRoute, 'client');
    
    // Staff/Employee active state detection
    $isStaffActive = in_array($currentRoute, ['agency.staff', 'agency.employee', 'agency.leaves', 'agecy.leaves']) || 
                    str_contains($currentRoute, 'staff') || 
                    str_contains($currentRoute, 'employee') || 
                    str_contains($currentRoute, 'leaves');
    
    // Team Management active state detection
    $isTeamActive = in_array($currentRoute, ['agency.team', 'teammanagment']) || 
                   str_contains($currentRoute, 'team');
    
    // Assignment active state detection
    $isAssignmentActive = in_array($currentRoute, ['agency.assignment']) || 
                         str_contains($currentRoute, 'assignment');
@endphp
...
{{-- </a> --}}
    ...
    @if($user_data->type != 'staff')
      <a href="{{route('agency_dashboard')}}">
        ...
        <div class="">
            <div onclick="toggleModule('servicesDiv','servicesArrow')" class="sidebar-item {{$isServicesActive ? 'border-gray-100/60 bg-secondary/90 parent-active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] cursor-pointer  relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                <div class="flex items-center">
                    <i class="fa fa-plane-departure mr-2 text-sm"></i>
                    <span class="text-lg font-medium">Services</span>
                </div>
                <i class="fa fa-caret-left text-2xl text-ternary absolute -right-1.5 top-[50%] translate-y-[-50%]"></i>
                <i class="fa fa-angle-down text-xl text-white/90 sidebar-arrow {{$isServicesActive ? '' : '-rotate-90'}} transition ease-in duration-2000 " id="servicesArrow"> </i>
            </div>
            <ul id="servicesDiv" class="pl-10 mt-2 flex flex-col sub-module-list {{$isServicesActive ? '' : 'hidden'}}">
            ...
            @if(isset($services) && $services->isNotEmpty())
                ...
                @foreach($services as $icon => $service)
                    ...
                    <li class="sidebar-item {{ Route::currentRouteName() === $service ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary' }}
     ...
     @if($visapermission)
        <div class="">
            ...
            <ul id="visaDiv" class="pl-10 mt-2 flex flex-col sub-module-list {{$isVisaActive ? '' : 'hidden'}}">
            <a href="{{ route('agency.application', ['type' => 'all']) }}">
                    <li class="sidebar-item {{Route::currentRouteName()==='agency.application' && (request('type') === 'all' || !request('type')) ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                        <div class="flex items-center">
                ...
                <a href="{{ route('agency.application', ['type' => 'pending']) }}">
                    <li class="sidebar-item {{Route::currentRouteName()==='agency.application' && request('type') === 'pending' ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                        <div class="flex items-center">
                            <i class="fa fa-eye mr-2 text-sm"></i>
        ...
        @endif
         ...
         @if($user_data->getAllPermissions()->pluck('name')->intersect(['requestform', 'manage everything'])->isNotEmpty())
        <a href="{{route('request.index')}}">
            <div class="{{Route::currentRouteName()==='request.index'?'border-gray-100/60 bg-secondary/90':'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                <div class="flex items-center">
                    <i class="fa fa-envelope-open-text mr-2 text-sm"></i>
                    <span class="text-lg font-medium">Request Form</span>
                </div>
                <i class="fa fa-caret-left text-2xl text-ternary absolute -right-1.5 top-[50%] translate-y-[-50%]"></i>
            </div>
        </a>

        <a href="{{route('flight.requests')}}">
            <div class="{{Route::currentRouteName()==='flight.requests'?'border-gray-100/60 bg-secondary/90':'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                <div class="flex items-center">
                    <i class="fa fa-plane mr-2 text-sm"></i>
                    <span class="text-lg font-medium">Flight Requests</span>
                </div>
                <i class="fa fa-caret-left text-2xl text-ternary absolute -right-1.5 top-[50%] translate-y-[-50%]"></i>
            </div>
        </a>
        ...
        <div class="">
            <div onclick="toggleModule('cancelinvoiceDiv','cancelArrow')" class="sidebar-item {{$isInvoiceActive ? 'border-gray-100/60 bg-secondary/90 parent-active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] cursor-pointer  relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                <div class="flex items-center">
                    <i class="fa-solid fa-file-invoice mr-2 text-sm"></i>
                    <span class="text-lg font-medium">Invoice</span>
                </div>
                <i class="fa fa-caret-left text-2xl text-ternary absolute -right-1.5 top-[50%] translate-y-[-50%]"></i>
                <i class="fa fa-angle-down text-xl text-white/90 sidebar-arrow {{$isInvoiceActive ? '' : '-rotate-90'}} transition ease-in duration-2000 " id="cancelArrow"> </i>
            </div>
            <ul id="cancelinvoiceDiv" class="pl-10 mt-2 flex flex-col sub-module-list {{$isInvoiceActive ? '' : 'hidden'}}">
            ...
            @php
                ...
                <li>
                    ...
                    class="sidebar-item {{ $isActive ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary' }}
       ...
       @endif
        ...
        @if($user_data->getAllPermissions()->pluck('name')->intersect(['booking view', 'manage everything'])->isNotEmpty())
            ...
            @if(in_array('Hotel', $availableRoutes) || in_array('Hotel', $availableRoutes) || in_array('superadminvisa.booking', $availableRoutes))
                <div>
                    <div onclick="toggleModule('bookingDiv','bookingArrow')"
                        class="sidebar-item {{$isBookingActive ? 'border-gray-100/60 bg-secondary/90 parent-active' : 'border-ternary'}}
                        ...
                        <i class="fa fa-caret-left text-2xl text-ternary absolute -right-1.5 top-[50%] translate-y-[-50%]"></i>
                        <i class="fa fa-angle-down text-xl text-white/90 sidebar-arrow {{$isBookingActive ? '' : '-rotate-90'}} transition ease-in duration-200" id="bookingArrow"></i>
                    </div>
                    ...
                    <ul id="bookingDiv" class="pl-10 mt-2 flex flex-col sub-module-list {{$isBookingActive ? '' : 'hidden'}}">
                        ...
                        @if(in_array('Visa', $availableRoutes))
                          <li class="sidebar-item {{ Route::currentRouteName() === 'agencyvisa.booking' ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary' }} w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60 hover:bg-secondary/90 transition ease-in duration-200">
        ...
        <div class="">
            <div onclick="toggleModule('staffDiv','staffArrow')" class="sidebar-item {{$isStaffActive ? 'border-gray-100/60 bg-secondary/90 parent-active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] cursor-pointer  relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                <div class="flex items-center">
                    <i class="fa fa-users mr-2 text-sm"></i>
            ...
            <a href="{{route('agency.staff')}}">
                    <li class="sidebar-item {{Route::currentRouteName()==='agency.staff'?'border-gray-100/60 bg-primary/90 active':'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                        <div class="flex items-center">
                            <i class="fa fa-eye mr-2 text-sm"></i>
                ...
                <li class="sidebar-item {{ Route::currentRouteName() === 'agency.staff.attandance' ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary' }} w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60 hover:bg-secondary/90 transition ease-in duration-2000">
                ...
                <li class="sidebar-item {{ Route::currentRouteName() === 'agency.staff.wages' ? 'border-gray-100/60 bg-primary/90 active' : 'border-ternary' }} w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60 hover:bg-secondary/90 transition ease-in duration-2000">
                ...
                <a href="{{ route('add.agency.leave', ['type' => 'agency']) }}">
                    <li class="sidebar-item {{Route::currentRouteName()==='add.agency.leave'?'border-gray-100/60 bg-primary/90 active':'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                        <div class="flex items-center">
                            <i class="fa fa-hotel mr-2 text-sm"></i>
                ...
                <a href="{{ route('agency.pending.leave', ['type' => 'agency']) }}">
                    <li class="sidebar-item {{Route::currentRouteName()==='agency.pending.leave'?'border-gray-100/60 bg-primary/90 active':'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                        <div class="flex items-center">
                            <i class="fa-brands fa-cc-visa mr-2 text-sm"></i>
        ...
        @if($user_data->getAllPermissions()->pluck('name')->intersect(['team', 'manage everything'])->isNotEmpty())
                   <a href="">
                        <div class="sidebar-item {{$isTeamActive ? 'border-gray-100/60 bg-secondary/90 parent-active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                            <div class="flex items-center">
         ...
         <a href="">
            <div class="sidebar-item {{$isAssignmentActive ? 'border-gray-100/60 bg-secondary/90 parent-active' : 'border-ternary'}}  w-full flex justify-between items-center py-2 px-4 rounded-[3px] text-white/90 border-[1px] border-b-[3px] border-r-[3px] relative hover:border-gray-100/60  hover:bg-secondary/90 transition ease-in duration-2000">
                <div class="flex items-center">
