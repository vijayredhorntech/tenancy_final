...
<table class="w-full border-[2px] border-secondary/40 border-collapse mt-4">
    ...
    @foreach($documents as $index => $document)
        ...
        <tr>
            ...
            <td class="border-[1px] border-secondary/50 px-4 py-1 text-ternary/80 font-medium text-sm">
                {{ $document->visaBookingApplication->application_number ?? '—' }}
            </td>      
            ...
            <td class="border-[1px] border-secondary/50 px-4 py-1 text-ternary/80 font-medium text-sm">
                <div class="flex gap-3 items-center justify-center">
                    ...
                    @else
                        {{-- <a href="{{ route('documents.view', ['document' => $document->related_id]) }}"
                           class="text-blue-600 hover:text-blue-800"
                           title="View Document">
                            <i class="fas fa-eye"></i>
                        </a> --}}
...
{{-- 
                    ...
                    @else
                        <a href="{{ route('downloade.signdocument', ['document' => $document->servicerelatedtableid]) }}"
                           class="text-purple-600 hover:text-purple-800"
