...
{{--        === this is code for form section ===--}}
             <div id="formDiv" class="w-full border-b-[2px] border-b-ternary/10 shadow-lg shadow-ternary/20 hidden">
                 ...
                 <form action="{{route('superadmin.storetermtype')}}" method="POST" enctype="multipart/form-data">
                 @csrf
                     ...
                     {{--               === textarea input field ===--}}
                         <div class="w-full relative group flex flex-col gap-1 xl:col-span-2">
                             ...
                             <div class="w-full relative">
                                  <input type="text" value="" name="name" placeholder="Enter term name "class="w-full pl-2 pr-8 py-1 rounded-[3px] rounded-tr-[8px] border-[1px] border-b-[2px] border-r-[2px] border-secondary/40 focus:outline-none focus:ring-0 focus:border-secondary/70 placeholder-ternary/70 transition ease-in duration-2000"> 
            ...
            <div class="w-full overflow-x-auto p-4">
                ...
                <table class="w-full border-[2px] border-secondary/40 border-collapse mt-4">
                    <tr>
                        ...
                        <td class="border-[2px] border-secondary/40 bg-gray-100 px-4 py-1.5 text-ternary/80 font-bold text-md">Term Name</td>
                        <td class="border-[2px] border-secondary/40 bg-gray-100 px-4 py-1.5 text-ternary/80 font-bold text-md">Total Terms</td>
                    ...
                    @forelse($termtypes as $type)
                        <tr class="{{$loop->iteration%2===0?'bg-gray-100/40':''}} hover:bg-secondary/10 cursor-pointer transition ease-in duration-2000" >
                            <td class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">{{$loop->iteration}}</td>
                            <td class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">{{$type['type']}}</td>
                            <td class="border-[2px] border-secondary/40 px-4 py-1 text-ternary/80 font-medium text-sm">
                                <div class="flex ">
                                    <span>{{ $type->terms->count() }}</span>
                                    <a href="{{route('superadmin.addterms',['id' => $type->id])}}" title="Add service">
                                        <div class="bg-success/10 text-success h-6 w-8 flex justify-center items-center rounded-[3px] hover:bg-success hover:text-white transition ease-in duration-200 ml-4">
                                            <i class="fa fa-plus"></i>
                                        </div>
                                    </a>

                                    <a href="{{route('superadmin.viewterms',['id' => $type->id])}}" title="Add service">
                                        <div class="bg-success/10 text-success h-6 w-8 flex justify-center items-center rounded-[3px] hover:bg-success hover:text-white transition ease-in duration-200 ml-4">
                                            <i class="fa fa-eye"></i>
                                        </div>
                                    </a>
                            ...
                            <td class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">
                            <span class="bg-{{$type['status']==='0'?'danger':'success'}}/10 text-{{$type['status']==='0'?'danger':'success'}} px-2 py-1 rounded-[3px] font-bold"> {{ $type['status'] == 1 ? 'Active' : 'Inactive' }}</span>
                             </td>
                            <td class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">{{$type['created_at']}}</td>
                            <td class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">
                                <div class="flex gap-2 items-center">
                                 ...
                                 {{--   <a href="{{route('superadmin.termedit',['id' => $type->id])}}" title="View Invoices"> --}}
