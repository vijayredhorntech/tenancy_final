            ...
            <div class="w-full overflow-x-auto p-4">
                <div class="w-full flex justify-between gap-2 items-center">
                     <div class="flex gap-2">
                         <a href="{{ route('superadmin.role.export.excel') }}?{{ http_build_query(request()->all()) }}" title="Export to excel" class="bg-success/20 text-success h-8 w-8 flex justify-center items-center rounded-[3px] hover:bg-success hover:text-white  cursor-pointer transition ease-in duration-2000">
                             <i class="fa fa-file-excel"></i>
                         </a>
                         <a href="{{ route('superadmin.role.export.pdf') }}?{{ http_build_query(request()->all()) }}" title="Export to pdf" class="bg-danger/20 text-danger h-8 w-8 flex justify-center items-center rounded-[3px] hover:bg-danger hover:text-white  cursor-pointer transition ease-in duration-2000">
                               <i class="fa fa-file-pdf"></i>
                         </a>
                     </div>
                    <div class="flex items-center gap-2">
                           <form method="GET" action="{{ route('superadmin.role') }}" class="flex items-center gap-2">
                               <input type="text" name="search" value="{{ request('search') }}" placeholder="Role name....." class="w-[200px] px-2 py-0.5 border-[1px] text-ternary border-success/80 placeholder-success rounded-l-[3px] focus:outline-none focus:ring-0 focus:border-success transition ease-in duration-2000" >
                               ...
                               <a href="{{ route('superadmin.role') }}" class="bg-gray-500 px-2 py-0.5 rounded-[3px] text-white font-bold border-[1px] border-gray-500 hover:bg-gray-600 transition ease-in duration-2000">
                                    Clear
                               </a>
                           </form>
                    </div>
                ...
                <table class="w-full border-[2px] border-secondary/40 border-collapse mt-4">
                        ...
                        <tr class="{{$loop->iteration%2===0?'bg-gray-100/40':''}} hover:bg-secondary/10 cursor-pointer transition ease-in duration-2000" >
                            ...
                            <td class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">
                                ...
                                {{$role->permissions->count()}}
                                     ...
                                     <a href="{{ route('superadmin_permissionassign', ['id' => $role->id]) }}">   <div  class=" bg-success/10 text-success h-6 w-6 flex justify-center items-center rounded-[3px] hover:bg-success hover:text-white transition ease-in duration-2000">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="border-[2px] border-secondary/40  px-4 py-1 text-ternary/80 font-medium text-sm">No Record Found</td>
                        </tr>
                    @endforelse
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <div>
                        ...
                        of {{ $roles->total() }} results (Page {{ $roles->currentPage() }} of {{ $roles->lastPage() }})
                    </div>
