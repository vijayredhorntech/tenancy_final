...
//     if ($totalPayment <= 0) {
//           $this->error('Please enter valid payment amounts.');
...
public function processPayment(Request $request)
{
    try {
        ...
        $agency = $this->agencyService->getAgencyData();
        if (!$agency) {
            $this->error('Unauthorized agency.');
            return back()->withInput()->with('reopen_pay_modal', true);
        }
        ...
        if (in_array('credit_card', $request->payment_methods) && $request->credit_card_amount > 0) {
            $totalPayment += $request->credit_card_amount;
            $paymentMethods['credit_card'] = $request->credit_card_amount;
        }

        if (in_array('debit_card', $request->payment_methods) && $request->debit_card_amount > 0) {
            $totalPayment += $request->debit_card_amount;
            $paymentMethods['debit_card'] = $request->debit_card_amount;
        }

        if (in_array('cash', $request->payment_methods) && $request->cash_amount > 0) {
            $totalPayment += $request->cash_amount;
            $paymentMethods['cash'] = $request->cash_amount;
        }

        if (in_array('bank_transfer', $request->payment_methods) && $request->bank_transfer_amount > 0) {
            $totalPayment += $request->bank_transfer_amount;
            $paymentMethods['bank_transfer'] = $request->bank_transfer_amount;
        }
         ...
         * Update statuses
         * ----------------------------- */
        $deduction->update(['agency_pay' => 'Paid']);
...
public function hsAllinvoice(Request $request)
{
 ...
 if ($request->filled('status') || $request->filled('date_from') || $request->filled('date_to') || $request->filled('search')) {
    ...
    $invoice = Deduction::with(['service_name', 'visaBooking.origin', 'visaBooking.destination', 'visaBooking.visasubtype', 'visaBooking.visa',  'invoice'])->findOrFail($id);
     $this->agencyService->getClientinfoById($invoice);
    // dd($invoice);
