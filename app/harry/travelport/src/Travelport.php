...
//    const BASE_URI = 'https://americas.universal-api.pp.travelport.com/B2BGateway/connect/uAPI/AirService';
    ...
    public function resetCredentials()
    {
        ...
        $this->TARGETBRANCH = 'P3710270';
        $this->CREDENTIALS ="Universal API/uAPI5981136332-0ea73c47:2Ht}Sx=5+T";
        $this->PROVIDER = 'P3710270';
    ...
    public function checkAirAvailability($routesArr)
    {
        ...
        // the flexi part is not passed here
    ...
    private function _apiCall($method, $postdata = [], $xml, $extraData = []){
        ...
        if(!empty($postdata))
        {
            ...
            // save to xml file
            $log_path = storage_path('logs'. DIRECTORY_SEPARATOR .'travelport_request.xml');
            $fp = fopen($log_path, 'a');
            fwrite($fp,$message);
            ...
            // save to xml file
            $log_path = storage_path('logs'. DIRECTORY_SEPARATOR .'travelport_response.xml');
            $fp = fopen($log_path, 'a');
            fwrite($fp,$response);
        }else{
            $response = array();
