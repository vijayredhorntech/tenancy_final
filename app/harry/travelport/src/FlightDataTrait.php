...
trait FlightDataTrait
{
    ...
    function getFlexiDepartureDates($deptime, $flexi = 3)
    {
        ...
        $flexi = (int) $flexi;
        $dates = [$deptime->format('Y-m-d')];
        for ($i = 1; $i <= $flexi; $i++) {
            $date = $deptime;
            $dates[] = $date->subdays(1)->format('Y-m-d');
        }
        return $dates;
    }

    function getFlexiReturnDates($deptime, $flexi = 3)
    {
        $deptime = Carbon::parse($deptime);
        $flexi = (int) $flexi;
        $dates = [$deptime->format('Y-m-d')];
        for ($i = 1; $i <= $flexi; $i++) {
            $date = $deptime;
            $dates[] = $date->adddays(1)->format('Y-m-d');
        }
        return $dates;
    }
                ...
                foreach ($flight_data[0] as $data) {
                    ...
                    foreach ($outbound as $outSegment) {
                        $temp = [
                            ...
                            'flightTimeFormatted' => $this->hoursandmins($this->convertObjectString($outSegment['FlightTime'])),
                            'distance' => $this->convertObjectString($outSegment['Distance']),
                            'availabilitySource' => $this->convertObjectString($outSegment['AvailabilitySource']),
                            'changeOfPlane' => $this->convertObjectString($outSegment['ChangeOfPlane']),
                            'eTicketability' => $this->convertObjectString($outSegment['ETicketability']),
                            'equipment' => $this->convertObjectString($outSegment['Equipment']),
                            'optionalServicesIndicator' => $this->convertObjectString($outSegment['OptionalServicesIndicator']),
                            'participantLevel' => $this->convertObjectString($outSegment['ParticipantLevel']),
                    ...
                    foreach ($inbound as $inSegment) {
                        $temp = [
                            ...
                            'flightTimeFormatted' => $this->hoursandmins($this->convertObjectString($inSegment['FlightTime'])),
                            'distance' => $this->convertObjectString($inSegment['Distance']),
                            'availabilitySource' => $this->convertObjectString($inSegment['AvailabilitySource']),
                            'changeOfPlane' => $this->convertObjectString($inSegment['ChangeOfPlane']),
                            'eTicketability' => $this->convertObjectString($inSegment['ETicketability']),
                            'equipment' => $this->convertObjectString($inSegment['Equipment']),
                            'optionalServicesIndicator' => $this->convertObjectString($inSegment['OptionalServicesIndicator']),
                            'participantLevel' => $this->convertObjectString($inSegment['ParticipantLevel']),
                    ...
                    $details->push([
                        ...
                        'outbound' => [
                            ...
                            'totalTimeTakenFromOriginToDestination' => $this->hoursandmins($outSegments->pluck('flightTime')->sum()),
                            'departureDate' => Carbon::parse($this->convertObjectString($outSegments->first()['departure']))->format('d F,Y'),
                            'departureTime' => Carbon::parse($this->convertObjectString($outSegments->first()['departure']))->format('h:i A') . ' (' . Carbon::parse($this->convertObjectString($outSegments->last()['arrival']))->format('H:i') . ')',
                        ...
                        'inbound' => [
                            ...
                            'totalTimeTakenFromOriginToDestination' => $this->hoursandmins($inSegments->pluck('flightTime')->sum()),

                            'departureDate' => Carbon::parse($this->convertObjectString($inSegments->first()['departure']))->format('d F,Y'),
                            'departureTime' => Carbon::parse($this->convertObjectString($inSegments->first()['departure']))->format('h:i A') . ' (' . Carbon::parse($this->convertObjectString($inSegments->last()['arrival']))->format('H:i') . ')',
                            ...
                            if (array_key_exists('air:AirItinerary', $jsons2)) {
                                ...
                                foreach ($jsons2['air:AirItinerary']['air:AirSegment'] as $g => $jsons5) {
                                    ...
                                    if (is_string($jsons5)) {
                                        ...
                                        if (strcmp($g, "@AvailabilitySource") == 0) {
                                            $details["AvailabilitySource"] = $jsons5;
                                        }
                                        if (strcmp($g, "@ChangeOfPlane") == 0) {
                                            $details["ChangeOfPlane"] = $jsons5;
                                        }
                                        if (strcmp($g, "@ETicketability") == 0) {
                                            $details["ETicketability"] = $jsons5;
                                        }
                                        if (strcmp($g, "@Equipment") == 0) {
                                            $details["Equipment"] = $jsons5;
                                        }
                                        if (strcmp($g, "@OptionalServicesIndicator") == 0) {
                                            $details["OptionalServicesIndicator"] = $jsons5;
                                        }
                                        if (strcmp($g, "@ParticipantLevel") == 0) {
                                            $details["ParticipantLevel"] = $jsons5;
                                        }
                                    } else {
                                        $details = [];
                                        foreach ($jsons5 as $k => $jsons6) {
                                            ...
                                            if (is_string($jsons6)) {
                                                ...
                                                if (strcmp($k, "@AvailabilitySource") == 0) {
                                                    $details["AvailabilitySource"] = $jsons6;
                                                }
                                                if (strcmp($k, "@ChangeOfPlane") == 0) {
                                                    $details["ChangeOfPlane"] = $jsons6;
                                                }
                                                if (strcmp($k, "@ETicketability") == 0) {
                                                    $details["ETicketability"] = $jsons6;
                                                }
                                                if (strcmp($k, "@Equipment") == 0) {
                                                    $details["Equipment"] = $jsons6;
                                                }
                                                if (strcmp($k, "@OptionalServicesIndicator") == 0) {
                                                    $details["OptionalServicesIndicator"] = $jsons6;
                                                }
                                                if (strcmp($k, "@ParticipantLevel") == 0) {
                                                    $details["ParticipantLevel"] = $jsons6;
                                                }
                                                // $details["changeofplane"] =$jsons6;
                                                // $details["optionalservicesindicator"]=$jsons6;
                                                // $details["availabilitysource"] =$jsons6;
